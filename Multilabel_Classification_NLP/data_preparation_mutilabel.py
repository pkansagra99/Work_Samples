...
def collate_batch(batch, vocab):
    ...
    texts = torch.cat(
        [torch.tensor(i, dtype=torch.int64) for i in list_of_list_of_indices]
    )
...
def get_loaders(trainset, validset, batch_size_, collate_fn):
    train_loader = torch.utils.data.DataLoader(
        trainset, batch_size=batch_size_, shuffle=True, collate_fn=collate_fn
    )
    valid_loader = torch.utils.data.DataLoader(
        validset, batch_size=batch_size_, shuffle=False, collate_fn=collate_fn
    )
...
def get_test_loaders(testset, batch_size_, collate_fn):
    test_loader = torch.utils.data.DataLoader(
        testset, batch_size=batch_size_, shuffle=False, collate_fn=collate_fn
    )
    return test_loader
